    ...
    <div className='bg-[#F8F8F8] pb-[85px] pt-[168px] grid justify-center grid-rows-1 grid-cols-[repeat(4,268px)] relative'>
      ...
      {MenuList.map((menu,i)=>(
            <div 
              ...
              className={`bg-[#F8F8F8] menu-section transition ease-in-out ${(i==0 || i == 2) ? 'duration-[3500ms]' : 'duration-[1500ms]'} ${visibleSections.includes(`section${i+1}`) ? 'opacity-100 translate-y-0' : `opacity-0 ${(i==0 || i == 2) ? '-translate-y-[400px]' : 'translate-y-[400px]'}`}`}
            >
